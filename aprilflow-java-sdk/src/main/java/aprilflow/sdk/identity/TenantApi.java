...
public final class TenantApi
{
    ...
    public TenantApi(AprilFlowHttpClient httpClient, JsonSerializer jsonSerializer)
    {
    ...
    public Tenant get()
    {
        ...
        response = ApiResponseHandler.requireSuccess(
            httpClient.execute(
                ApiRequest.get("/identity/v1/pub/tenants")
                    .build()
                    .toHttpRequest()
            )
        );

        return jsonSerializer.deserialize(response.body(), Tenant.class);
    }
}
