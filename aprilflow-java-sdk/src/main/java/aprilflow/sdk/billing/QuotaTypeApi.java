...
public final class QuotaTypeApi
{
    private final AprilFlowHttpClient httpClient;
    private final JsonSerializer jsonSerializer;

    public QuotaTypeApi(AprilFlowHttpClient httpClient, JsonSerializer jsonSerializer)
    {
    ...
    public List<QuotaType> list()
    {
        HttpResponse response;

        response = ApiResponseHandler.requireSuccess(
            httpClient.execute(
                ApiRequest.get("/billing/v1/pub/quotas/types")
                    .build()
                    .toHttpRequest()
            )
        );
        ...
        return jsonSerializer.deserialize(
            response.body(),
            new TypeReference<List<QuotaType>>()
            {
            }
        );
    }
